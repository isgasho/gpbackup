...
func GetPartitionAlteredSchema(connectionPool *dbconn.DBConn) map[uint32][]AlteredPartitionRelation {
	...
	FROM pg_catalog.pg_partition_rule pgpr
		LEFT JOIN pg_catalog.pg_partition pgp ON pgp.oid = pgpr.paroid
		LEFT JOIN pg_catalog.pg_class pgc ON pgpr.parchildrelid = pgc.oid
		LEFT JOIN pg_catalog.pg_class pgc2 ON pgp.parrelid = pgc2.oid
		LEFT JOIN pg_catalog.pg_namespace pgn ON pgc.relnamespace = pgn.oid
		LEFT JOIN pg_catalog.pg_namespace pgn2 ON pgc2.relnamespace = pgn2.oid
